<?php

declare(strict_types=1);

/*
 * This file has been auto generated by Jane,
 *
 * Do no edit it directly.
 */

namespace Bitly\Model;

class GroupPreferences extends \ArrayObject
{
    /**
     * @var array
     */
    protected $initialized = [];

    public function isInitialized($property): bool
    {
        return array_key_exists($property, $this->initialized);
    }
    /**
     * @var string
     */
    protected $groupGuid;
    /**
     * @var string
     */
    protected $domainPreference;

    public function getGroupGuid(): string
    {
        return $this->groupGuid;
    }

    public function setGroupGuid(string $groupGuid): self
    {
        $this->initialized['groupGuid'] = true;
        $this->groupGuid = $groupGuid;

        return $this;
    }

    public function getDomainPreference(): string
    {
        return $this->domainPreference;
    }

    public function setDomainPreference(string $domainPreference): self
    {
        $this->initialized['domainPreference'] = true;
        $this->domainPreference = $domainPreference;

        return $this;
    }
}
